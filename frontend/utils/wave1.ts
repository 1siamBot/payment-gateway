...
export type EvidencePackageArtifactType =
  | 'timeline'
  | 'playbook'
  | 'digest'
  | 'checklist'
...
export type EvidencePackageDiffChangeType = 'added' | 'removed' | 'modified' | 'unchanged';
export type EvidencePackageDiffStudioShortcut =
  | 'focus_diff_studio'
  | 'next_diff'
  | 'prev_diff'
  | 'open_qa_preflight'
...
export type EvidencePackageDiffEntry = {
  issueIdentifier: string;
  severityWeight: number;
  packageSectionWeight: number;
  artifactType: EvidencePackageArtifactType;
  artifactPath: string;
  checksum: string;
};

export type EvidencePackageDiffStudioRow = {
  id: string;
  issueIdentifier: string;
  severityWeight: number;
  packageSectionWeight: number;
  artifactType: EvidencePackageArtifactType;
  artifactPath: string;
  changeType: EvidencePackageDiffChangeType;
  baselineChecksum: string;
  candidateChecksum: string;
};

export type EvidencePackageDiffStudio = {
  contract: 'settlement-evidence-package-diff-studio.v1';
  rows: EvidencePackageDiffStudioRow[];
  changeCounts: Record<EvidencePackageDiffChangeType, number>;
  activeRowId: string;
};

export type QaPreflightViolationSeverity = 'error' | 'warning';

export type QaPreflightLinkViolation = {
  link: string;
  reason: string;
  severity: QaPreflightViolationSeverity;
};

export type QaHandoffPreflightSummary = {
  contract: 'settlement-qa-handoff-preflight.v1';
  missingEvidence: string[];
  linkViolations: QaPreflightLinkViolation[];
  dependencyViolations: string[];
  readyForQa: boolean;
  stableSummaryJson: string;
};

export type QaCanonicalLinkAutofixRow = {
  id: string;
  original: string;
  normalized: string;
  changed: boolean;
...
export type QaCanonicalLinkAutofixPreview = {
  contract: 'settlement-qa-canonical-link-autofix.v1';
  rows: QaCanonicalLinkAutofixRow[];
  correctedOutput: string;
  changedCount: number;
  invalidCount: number;
};
...
const EVIDENCE_PACKAGE_ARTIFACT_ORDER: Record<EvidencePackageArtifactType, number> = {
  timeline: 1,
  playbook: 2,
  digest: 3,
  checklist: 4,
...
const EVIDENCE_PACKAGE_DIFF_CHANGE_ORDER: Record<EvidencePackageDiffChangeType, number> = {
  added: 1,
  removed: 2,
  modified: 3,
  unchanged: 4,
...
function normalizeEvidencePackageArtifactType(input: unknown): EvidencePackageArtifactType {
  if (typeof input !== 'string') {
    return 'other';
  }
  const normalized = input.trim().toLowerCase();
...
function parseEvidencePackageDiffEntry(raw: unknown): EvidencePackageDiffEntry | null {
  if (!raw || typeof raw !== 'object') {
    return null;
  }
  const row = raw as Record<string, unknown>;
  ...
  if (!issueIdentifier || !artifactPath) {
    return null;
  }
  const severityWeight = Math.max(0, Math.trunc(parseFiniteNumber(row.severityWeight) ?? 0));
  const packageSectionWeight = Math.max(0, Math.trunc(parseFiniteNumber(row.packageSectionWeight) ?? 0));
  const artifactType = normalizeEvidencePackageArtifactType(row.artifactType);
  const checksum = normalizeOptional(
...
function evidencePackageDiffRowKey(
  row: Pick<EvidencePackageDiffEntry, 'issueIdentifier' | 'artifactType' | 'artifactPath'>,
): string {
  return `${row.issueIdentifier}|${row.artifactType}|${row.artifactPath}`;
}

export function buildEvidencePackageDiffStudio(input: {
  baselinePackage: unknown[];
  candidatePackage: unknown[];
  activeRowId: string;
}): EvidencePackageDiffStudio {
  const baselineRows = input.baselinePackage
    .map((row) => parseEvidencePackageDiffEntry(row))
    .filter((row): row is EvidencePackageDiffEntry => Boolean(row));
  const candidateRows = input.candidatePackage
    .map((row) => parseEvidencePackageDiffEntry(row))
    .filter((row): row is EvidencePackageDiffEntry => Boolean(row));
  const baselineMap = new Map(baselineRows.map((row) => [evidencePackageDiffRowKey(row), row] as const));
  const candidateMap = new Map(candidateRows.map((row) => [evidencePackageDiffRowKey(row), row] as const));
  const rowKeys = Array.from(new Set([
    ...baselineMap.keys(),
    ...candidateMap.keys(),
  ]));
  const rows = rowKeys
    .map((key) => {
      ...
      let changeType: EvidencePackageDiffChangeType = 'unchanged';
      if (!baseline && candidate) {
        changeType = 'added';
      } else if (baseline && !candidate) {
        changeType = 'removed';
      } else if ((baseline?.checksum ?? '') !== (candidate?.checksum ?? '')) {
        changeType = 'modified';
      }

      const issueIdentifier = baseline?.issueIdentifier ?? candidate?.issueIdentifier ?? '';
      const artifactType = baseline?.artifactType ?? candidate?.artifactType ?? 'other';
      const artifactPath = baseline?.artifactPath ?? candidate?.artifactPath ?? '';
      const severityWeight = baseline?.severityWeight ?? candidate?.severityWeight ?? 0;
      const packageSectionWeight = baseline?.packageSectionWeight ?? candidate?.packageSectionWeight ?? 0;
      return {
        id: `${issueIdentifier}|${artifactType}|${artifactPath}`,
        issueIdentifier,
        severityWeight,
        packageSectionWeight,
        artifactType,
      ...
      } satisfies EvidencePackageDiffStudioRow;
    })
    .filter((row): row is EvidencePackageDiffStudioRow => Boolean(row))
    .sort((left, right) => (
      left.severityWeight - right.severityWeight
      || left.packageSectionWeight - right.packageSectionWeight
      || left.issueIdentifier.localeCompare(right.issueIdentifier)
      || EVIDENCE_PACKAGE_ARTIFACT_ORDER[left.artifactType] - EVIDENCE_PACKAGE_ARTIFACT_ORDER[right.artifactType]
      || left.artifactPath.localeCompare(right.artifactPath)
      || EVIDENCE_PACKAGE_DIFF_CHANGE_ORDER[left.changeType] - EVIDENCE_PACKAGE_DIFF_CHANGE_ORDER[right.changeType]
      || left.id.localeCompare(right.id)
    ));
  const rowIdSet = new Set(rows.map((row) => row.id));
  const activeRowId = rowIdSet.has(input.activeRowId)
    ? input.activeRowId
    : (rows[0]?.id ?? '');
  const changeCounts: Record<EvidencePackageDiffChangeType, number> = {
    added: 0,
    removed: 0,
    modified: 0,
    unchanged: 0,
...
export function moveEvidencePackageDiffSelection(input: {
  rows: EvidencePackageDiffStudioRow[];
  activeRowId: string;
  direction: 'next' | 'prev';
}): string {
...
export function resolveEvidencePackageDiffStudioShortcut(input: {
  key: string;
  altKey?: boolean;
  shiftKey?: boolean;
  ctrlKey?: boolean;
  metaKey?: boolean;
}): EvidencePackageDiffStudioShortcut | null {
  const normalizedKey = input.key.toLowerCase();
  if (input.altKey && normalizedKey === 'd') {
    return 'focus_diff_studio';
  }
  if (input.altKey && input.shiftKey && normalizedKey === 'n') {
    return 'next_diff';
  }
  ...
  if (normalizedKey === 'q' && input.shiftKey && (input.ctrlKey || input.metaKey)) {
    return 'open_qa_preflight';
  }
  if (input.key === 'Enter' && input.shiftKey && (input.ctrlKey || input.metaKey)) {
...
function normalizeCanonicalQaLink(input: string): string | null {
  const canonical = canonicalizePaperclipIssueLink(input);
  if (!canonical) {
    return null;
  }
  if (!/^\/[A-Z0-9_-]+\/issues\/[A-Z0-9-]+(?:#(comment-[a-z0-9-]+|document-[a-z0-9_-]+))?$/i.test(canonical)) {
    return null;
  }
  return canonical;
}

export function buildQaCanonicalLinkAutofixPreview(input: {
  links: string[];
}): QaCanonicalLinkAutofixPreview {
  const rows = input.links.map((raw, index) => {
    const original = raw.trim();
    const normalized = normalizeCanonicalQaLink(original);
    const reason = normalized
      ? 'ok'
      : 'Link must be /<PREFIX>/issues/<IDENTIFIER> with optional #comment-... or #document-... anchor.';
    return {
      id: `qa-canonical-${index + 1}`,
      original,
      normalized: normalized ?? original,
      changed: Boolean(normalized && normalized !== original),
      valid: Boolean(normalized),
      reason,
    } satisfies QaCanonicalLinkAutofixRow;
  });
...
export function buildQaHandoffPreflightSummary(input: {
  requiredEvidence: string[];
  providedEvidence: string[];
  dependencyIssueLinks: string[];
  exportLinks: string[];
}): QaHandoffPreflightSummary {
  const requiredEvidence = Array.from(new Set(normalizeMultilineEntries(input.requiredEvidence.join('\n'))))
    .sort((left, right) => left.localeCompare(right));
  const providedEvidence = new Set(normalizeMultilineEntries(input.providedEvidence.join('\n')));
  const missingEvidence = requiredEvidence.filter((path) => !providedEvidence.has(path));
  const linkPreview = buildQaCanonicalLinkAutofixPreview({ links: input.exportLinks });
  const linkViolations = linkPreview.rows
    .filter((row) => !row.valid)
    .map((row) => ({
      link: row.original,
      reason: row.reason,
      severity: 'error',
    } satisfies QaPreflightLinkViolation));

  const dependencyViolations: string[] = [];
  const dependencyLinks = normalizeMultilineEntries(input.dependencyIssueLinks.join('\n'));
  if (dependencyLinks.length === 0) {
    dependencyViolations.push('At least one dependency issue link is required.');
  }
  dependencyLinks.forEach((link) => {
    if (!normalizeCanonicalQaLink(link)) {
      dependencyViolations.push(`Dependency issue link is non-canonical: ${link}`);
    }
  });

  const readyForQa = missingEvidence.length === 0
    && linkViolations.length === 0
    && dependencyViolations.length === 0;
  const stableSummaryJson = serializeDeterministicUnknown({
    missingEvidence,
    linkViolations,
